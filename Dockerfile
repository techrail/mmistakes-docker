...
ENV PATH="${PATH}:/root/.rbenv/bin:/root/.rbenv/versions/3.3.8/bin"
...
RUN apt update && \
	...
	~/.rbenv/bin/rbenv install -fv 3.3.8

# Install bundler and use it to install minimal-mistakes jekyll theme and dependencies
RUN ~/.rbenv/versions/3.3.8/bin/gem install bundler
RUN ~/.rbenv/versions/3.3.8/bin/bundler install
